...
test('re-identify revokes previous presence on same connection', async () => {
  const { port, server, room } = await setupServer()
  const ws = new WebSocket(`ws://localhost:${port}/ws`)
  const initial = nextMessage(ws)
  await Promise.all([opened(ws), initial])
  ...
  ws.send(JSON.stringify({ event: 'identify', payload: { who: 'Codex' } }))
  const msg = await nextMessage(ws)
  assert.equal(msg.event, 'presence')
  assert.equal(msg.state.presence.Code, false)
  assert.equal(msg.state.presence.Codex, true)
  assert.equal(room.presence.Code, false)
  assert.equal(room.presence.Codex, true)
  ws.close()
  await closedServer(server)
})
