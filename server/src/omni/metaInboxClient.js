...
export async function checkMetaConnectorHealth({ fetchImpl = fetch, monitoredProfiles = null } = {}) {
  ...
  const profileKeys = (monitoredProfiles || String(process.env.OMNI_META_HEALTH_PAGE_PROFILES || 'anna_lynn,man_kynd,page_des,tangtob,fb_112154661515664,vz_viris_zamara')
    .split(',')
    .map((item) => item.trim())
    .filter(Boolean))
